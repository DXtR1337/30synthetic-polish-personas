...
def call_azure_gpt(system, user):
    ...
    if not key:
        raise RuntimeError('AZURE_OPENAI_KEY environment variable is required')
...
def call_azure_gpt55(system, user):
    key = os.environ.get('AZURE_OPENAI_KEY')
    if not key:
        raise RuntimeError('AZURE_OPENAI_KEY environment variable is required')
    deployment = os.environ.get('AZURE_OPENAI_DEPLOYMENT_GPT55', 'gpt-5.5')
    url = f'https://synthetic-benchmark.openai.azure.com/openai/deployments/{deployment}/chat/completions?api-version=2024-10-21'
    body = json.dumps({'messages':[{'role':'system','content':system},{'role':'user','content':user}],'max_completion_tokens':16000}).encode()
    req = urllib.request.Request(url, data=body, headers={'Content-Type':'application/json','api-key':key}, method='POST')
    with urllib.request.urlopen(req, timeout=300) as resp:
        r = json.loads(resp.read())
...
if __name__ == '__main__':
    ...
    models = {
        ...
        'gpt55': lambda s,u: call_azure_gpt55(s,u),
