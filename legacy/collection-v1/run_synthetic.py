...
def load_tctm_vignettes():
    ...
    for entry in entries[1:]:  # pierwszy to preamble
        ...
        if msgs_block:
            ...
            # sender zawiera tylko litery/cyfry/spacje — bezpieczne [^']*.
            # text może zawierać escape'y (np. "deadline'u"), używamy patternu
            # tolerującego \\' wewnątrz stringu.
            for line in re.finditer(
                r"\{\s*sender:\s*'([^']*)',\s*text:\s*'((?:[^'\\]|\\.)*)'(?:,\s*isSystem:\s*(true|false))?(?:,\s*isMedia:\s*(true|false))?\s*\}",
                msgs_raw
            ):
                sender, text, is_sys, _ = line.group(1), line.group(2), line.group(3), line.group(4)
                messages.append({
                    'sender': sender,
                    'text': text.replace("\\'", "'").replace('\\"', '"'),
                    'isSystem': is_sys == 'true',
                })
